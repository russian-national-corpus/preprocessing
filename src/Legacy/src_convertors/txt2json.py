...
from src.Legacy.src_convertors.json2cg import JSON2CG
from src.Legacy.src_convertors.simple_convertors.text_processor import TextProcessor
...
class Txt2JSON:
    ...
    def __init__(self, settingsDir='conf'):
        ...
        self.settingsDir = os.path.join(settingsDir, '0')
        self.load_settings()

        self.corpusSettings['corpus_dir'] = os.path.join(self.corpusSettings['corpus_dir'], self.corpusSettings['corpus_name'])

        # this manipulation is needed to implement the logic of tsakorpus conf directory substitution
        self.settingsDir = os.path.join(settingsDir, '1')
        self.load_settings()
    ...
    def process_corpus(self, source_directory: Path, target_directory: Path) -> None:
        """
        ...
        for path, dirs, files in os.walk(str(source_directory)):
            for filename in files:
                if not filename.lower().endswith('.' + self.srcExt):
                    continue
                targetPath = path.replace(str(source_directory), str(target_directory))
                if targetPath == path:
